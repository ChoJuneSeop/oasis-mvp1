...
function runTrial(model, seed) {
  ...
  // Shared world tape: every model receives the same encounter order for the same seed.
  for (const npc of encounterOrder) {
    seen.add(npc);
    if (model.rel && model.feedback) relations.add(npc);

    // Ordinary gated possibility. External baselines may also open it from observation;
    // NoRelation cannot use relationship history to open it.
  ...
  // Relationship-composed hidden possibility: requires a relation process, not mere NPC visibility.
  ...
  if (hiddenReady) {
    s.hiddenGenerated++;
    s.hiddenCandidate++;
    // Deterministic but seed-sensitive single choice; only one path is realized.
    if (rand() > 0.18) {
      s.hiddenSelected++;
      s.hiddenRealized++;
