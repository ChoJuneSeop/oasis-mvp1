...
try{
  ...
  const result=await page.evaluate(({MAX_TICK,SURROGATE_COUNT})=>{
    ...
    choose=function(S0,P){if(S0===S&&MODELS[S0.key].kind==='oasis'){const L=P.relationField?.latent;if(L&&L.byId?.size){const ids=completedIds(P,I.candidates(S,P)),noRows=evalRows(P,[]),yesRows=evalRows(P,ids),a=sig(yesRows),b=sig(noRows),r=structural(P,ids);r.choiceDiff=a.choice!==b.choice;r.decisionDiff=changedSig(a,b);records.push(r)}}baseChoose(S0,P)};
