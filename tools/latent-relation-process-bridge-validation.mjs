...
const TMP='tools/.tmp-oasis-latent-process-bridge-validation.mjs';
const MAX_TICK=Number(process.env.OASIS_PROCESS_BRIDGE_MAX_TICK||30000);
...
const newEvidence=`function currentRelationEvidence(S,P,ep,id,L){
  ...
  for(let i=recentEpisodes.length-1;i>=0;i--){
    ...
    const sharedParticipant=q.a===ep.a||q.a===ep.b||q.b===ep.a||q.b===ep.b;
    const qCurrent=q.places.includes(here)||q.places.includes(target)||(gate&&(q.a===gate||q.b===gate));
    if(sharedParticipant&&qCurrent){bridge=q;break;}
  }
  const reasons=[];if(directHere)reasons.push('direct-here:'+here);if(directTarget)reasons.push('direct-target:'+target);if(gateMatch)reasons.push('direct-gate:'+gate);if(bridge)reasons.push('completed-process-bridge:'+bridge.t+':'+bridge.key);
  return {direct,bridgeValid:!!bridge,bridgeTick:bridge?.t??null,bridgeKey:bridge?.key??null,reasons};
}`;
...
const newStats="function gateStats(L){return L.gateStats||(L.gateStats={evaluations:0,retrievedAdds:0,retrievedReleases:0,rejectedAdds:0,processBridgeBlocked:0,retrievalAddXor:0,retrievalReleaseXor:0,rejectXor:0})}";
...
const newActive="const prev=new Set(L.activeIds||[]),active=[];for(const x of retrieved){const evidence=currentRelationEvidence(S,P,x.ep,x.id,L);const admitted=prev.has(x.id)?evidence.direct:(evidence.direct&&evidence.bridgeValid);if(admitted)active.push({...x,evidence,admissionMode:prev.has(x.id)?'retain':'completed-process-entry'});else{if(evidence.direct&&!evidence.bridgeValid&&!prev.has(x.id))G.processBridgeBlocked++;if(!prevRetrieved.has(x.id)){G.rejectedAdds++;G.rejectXor=(G.rejectXor^gateHash(x.id))>>>0;}}}\n  const now=new Set(active.map(x=>x.id));";
...
const newReact="for(const x of active)if(!prev.has(x.id))audit(P,'reactivate',{episodeId:x.id,key:x.ep.key,createdTick:x.ep.t,age:E.tick-x.ep.t,retrievalReasons:x.reasons,evidenceReasons:x.evidence.reasons,admissionMode:x.admissionMode,bridgeTick:x.evidence.bridgeTick,bridgeKey:x.evidence.bridgeKey,from:[...(x.ep.from||[])],places:[...x.ep.places]});";
...
src=src
  .replace("const PORT=4187, MAX_TICK=120000, CHUNK=1000;",`const PORT=4190, MAX_TICK=${MAX_TICK}, CHUNK=1000;`)
  .replace("const AUDIT_FILE='latent-relation-admission-fast-audit.jsonl';","const AUDIT_FILE='latent-relation-process-bridge-audit.jsonl';")
  .replace("const CF_FILE='latent-relation-admission-fast-counterfactual.jsonl';","const CF_FILE='latent-relation-process-bridge-counterfactual.jsonl';")
  .replace("const REPORT_FILE='latent-relation-admission-fast-report.json';","const REPORT_FILE='latent-relation-process-bridge-report.json';")
  .replace("totalGateEvaluations:retrievalTotals.evaluations||0,","totalGateEvaluations:retrievalTotals.evaluations||0,totalProcessBridgeBlocked:retrievalTotals.processBridgeBlocked||0,")
  .replace("purpose:'efficient exact separation of latent retrieval from actual current relation admission'","purpose:'reactivate only when a newly completed relational process bridges a latent process into the current flow'")
  .replace("gate:'direct current place/target/gate clue AND recent-18 NPC process anchor'","gate:'latent process must be directly current-relevant and connect by shared participant to a newly completed relation-field episode that is itself current-relevant; re-entry requires such episode after last exit'")
  ...
  .replace("if(summary.totalReactivations>=BASELINE.reactivations)throw new Error('churn not reduced');","if(MAX_TICK===30000&&summary.totalReactivations>=25854)throw new Error('process bridge did not reduce raw-bridge 30k reactivations');if(MAX_TICK===120000&&summary.totalReactivations>=752449)throw new Error('process bridge did not reduce raw-bridge 120k reactivations');if(summary.directCounterfactualDifferences<=0)throw new Error('no retained decision contribution');if(summary.choiceDifferences<=0)throw new Error('no retained choice contribution');")
  .replace("console.log('RESULT fast admission-gate validation completed; production and reality engine unchanged.');","console.log('RESULT completed-process bridge validation completed; production and reality engine unchanged.');");

for(const marker of ['lastExit=new Map','processBridgeBlocked','completed-process-bridge','completed-process-entry'])if(!src.includes(marker))throw new Error('missing transformed marker '+marker);
