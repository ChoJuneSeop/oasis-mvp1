...
const TMP='tools/.tmp-latent-relation-responsibility-phase-shift.mjs';
let src=await readFile(BASE,'utf8');
src=src
  .replace("const PORT=4192, MAX_TICK=30000, CHUNK=1000;","const PORT=4198, MAX_TICK=30000, CHUNK=1000;")
  .replace("const REPORT_FILE='latent-relation-shadow-gate-comparison-report.json';","const REPORT_FILE='latent-relation-responsibility-phase-shift-report.json';")
  .replace("const names=['broad','rawBridge','completedProcess','exactProvenance'];","const names=['broad','rawBridge','completedProcess','exactProvenance','lag0','lag1','lag2','lag3'];")
  .replace("G.byParty.set(P.id,{active:new Set(),lastExit:new Map()})","G.byParty.set(P.id,{active:new Set(),lastExit:new Map(),lastDanger:null,riseHistory:[]})");
...
const gateFn=`    function gateIds(name,P,retrieved){
      ...
      const phase=name.startsWith('lag')?Number(name.slice(3)):null;
      const adaptive=phase!=null&&Number.isFinite(phase);
      const rising=adaptive&&st.lastDanger!=null&&S.danger>st.lastDanger;
      let useCompleted=false;
      if(adaptive){
        if(phase===0)useCompleted=rising;
        else useCompleted=st.riseHistory.length>=phase?!!st.riseHistory[st.riseHistory.length-phase]:false;
      }
      ...
      if(adaptive){
        stats.completedModeEvaluations=(stats.completedModeEvaluations||0)+(useCompleted?1:0);
        stats.exactModeEvaluations=(stats.exactModeEvaluations||0)+(useCompleted?0:1);
      }
...
src=src.replace("shadowGates:['broad','rawBridge','completedProcess','exactProvenance']","shadowGates:['broad','rawBridge','completedProcess','exactProvenance','lag0','lag1','lag2','lag3']");
...
const extra=[
  targetFinal,
  "  const phase=Object.fromEntries(['lag0','lag1','lag2','lag3'].map(n=>{const s=final.gates[n];return[n,{entries:s.entries,meanActive:s.meanActive,choiceDiffs:s.choiceDiffs,decisionDiffs:s.decisionDiffs,choiceYield:s.choiceYield,decisionYield:s.decisionYield,completedModeEvaluations:s.completedModeEvaluations,exactModeEvaluations:s.exactModeEvaluations}]}));",
  "  const ranked=Object.entries(phase).sort((a,b)=>b[1].choiceYield-a[1].choiceYield||b[1].choiceDiffs-a[1].choiceDiffs||a[1].meanActive-b[1].meanActive).map(([n])=>n);",
  "  console.log('PHASE-SHIFT '+JSON.stringify({phase,ranked,best:ranked[0]}));"
].join('\n');
if(!src.includes(targetFinal))throw new Error('final insertion target missing');
src=src.replace(targetFinal,extra);
if(!src.includes("'lag3'")||!src.includes('PHASE-SHIFT'))throw new Error('phase-shift transform failed');
