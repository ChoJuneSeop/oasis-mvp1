...
const PORT=4490+Number(process.env.OFFSET||0)%100;
const HORIZON=120000;
const OFFSET=Number(process.env.OFFSET||0);
const LABEL=process.env.LABEL||`offset${OFFSET}`;
const REPORT=`flow-selective-joint-response-v1.2-r4-${LABEL}.json`;
const expected={
  0:{dawn:917,star:547,blue:533},137:{dawn:593,star:757,blue:535},977:{dawn:531,star:543,blue:548},4099:{dawn:672,star:760,blue:577}
}[OFFSET]||null;
...
try{
  ...
  const report={experiment:'OASIS Flow-Selective Joint Response v1.2',iteration:'r4',stage:'A-longitudinal-regression',label:LABEL,offset:OFFSET,repairClass:'measurement-condition-alignment',sourceAudit,validity,pass:Object.values(validity).every(Boolean),result};
  await writeFile(REPORT,JSON.stringify(report,null,2));console.log('OASIS-FSJR-R4',JSON.stringify({label:LABEL,pass:report.pass,validity,finalCounts:result.finalCounts,snapshots:result.snapshots,final:result.final}));if(!report.pass)process.exitCode=1;
