...
const PORT=4207, MAX_TICK=120000;
const OUT='o3-danger-balance-records-120k.json';
...
try{
  ...
  const result=await page.evaluate(({MAX_TICK})=>{
    ...
    const summary={maxTick:MAX_TICK,records:records.length,choiceDiffs:records.filter(r=>r.choiceDiff).length,decisionDiffs:records.filter(r=>r.decisionDiff).length,partyCounts:Object.fromEntries(Object.entries(byParty).map(([k,v])=>[k,v.length])),world:{actions:S.c.actions,relationEvents:S.c.rel,recombinations:S.c.relationRecombination},npcNames,placeIds,records};
    E=savedE;delete globalThis.__OASIS_SHADOW_INTERNALS;return summary;
  },{MAX_TICK});

  if(result.records.length!==1997||result.choiceDiffs!==164||result.decisionDiffs!==197)throw new Error(`120k canonical mismatch ${JSON.stringify({records:result.records.length,choiceDiffs:result.choiceDiffs,decisionDiffs:result.decisionDiffs})}`);
  await writeFile(OUT,JSON.stringify({design:{purpose:'export the fixed 120k production trajectory for balance-constrained matching; no matching is performed here',worldPolicy:'production trajectory unchanged; O3 completed-process shadow comparison fixed',dangerPolicy:'danger recorded only for post-match outcome analysis and must not be used to construct matches'},...result},null,2));
  console.log(`O3-DANGER-BALANCE-EXPORT ${JSON.stringify({records:result.records.length,choiceDiffs:result.choiceDiffs,decisionDiffs:result.decisionDiffs,partyCounts:result.partyCounts,world:result.world})}`);
  await context.close();
