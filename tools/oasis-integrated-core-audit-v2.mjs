...
import { createOASIS } from '../src/oasis-integrated-core.mjs';
...
{
  const o = createOASIS();
  o.observe(event('memory-action-1', {
    entities: ['OASIS', 'A'],
    participants: [{ id: 'OASIS', capabilities: ['observe'] }],
    relations: [{ id: 'memory-rel', from: 'OASIS', to: 'A', kind: 'observes' }],
    affordances: [{ id: 'observe-once', actor: 'OASIS', action: 'observe', target: 'A' }]
  }));
  const d1 = o.deliberate();
  o.actualize(d1.choice.id, event('memory-outcome-1', {
    entities: ['OASIS', 'A'],
    relations: [{ id: 'memory-outcome-rel', from: 'A', to: 'OASIS', kind: 'became-known' }],
    affordances: [{ id: 'observe-once', op: 'remove' }]
  }));
  o.observe(event('memory-return', {
    entities: ['A'],
    participants: [{ id: 'OASIS', capabilities: ['observe'] }]
  }));
  const d2 = o.deliberate();
...
{
  const o = createOASIS();
  o.observe(event('nonrecursive-1', {
    entities: ['OASIS', 'A'],
    participants: [{ id: 'OASIS', capabilities: ['observe', 'communicate'] }],
    relations: [{ id: 'old-only', from: 'OASIS', to: 'A', kind: 'old-relation' }],
    affordances: [{ id: 'observe-old', actor: 'OASIS', action: 'observe', target: 'A' }]
  }));
  const d1 = o.deliberate();
  o.actualize(d1.choice.id, event('nonrecursive-outcome-1', {
    entities: ['A'],
    relations: [{ id: 'old-outcome-only', from: 'A', to: 'OASIS', kind: 'old-outcome' }],
    affordances: [{ id: 'observe-old', op: 'remove' }]
  }));
  o.observe(event('remove-old-relations', {
    entities: ['A'],
    relations: [
      { id: 'old-only', from: 'OASIS', to: 'A', kind: 'old-relation', op: 'remove' },
      { id: 'old-outcome-only', from: 'A', to: 'OASIS', kind: 'old-outcome', op: 'remove' }
    ],
    affordances: [{ id: 'communicate-new', actor: 'OASIS', action: 'communicate', target: 'A' }]
  }));
  const d2 = o.deliberate();
  assert.equal(d2.field.reactivatedExperienceIds.includes('experience:0'), true);
  const exp2 = o.actualize(d2.choice.id, event('nonrecursive-outcome-2', {
    entities: ['A', 'B'],
    relations: [{ id: 'new-only', from: 'A', to: 'B', kind: 'new-relation' }],
    affordances: [{ id: 'communicate-new', op: 'remove' }]
  }));
  assert.equal(exp2.processRelations.some(r => r.id === 'old-only' || r.id === 'old-outcome-only'), false);
}

{
  const o = createOASIS();
  o.observe(event('irrelevant-base', {
    entities: ['X', 'Y'],
    relations: [{ id: 'xy-unrelated', from: 'X', to: 'Y', kind: 'unrelated' }]
  }));
  o.observe(event('relevant-now', {
    entities: ['OASIS', 'A'],
    participants: [{ id: 'OASIS', capabilities: ['observe'] }],
    relations: [{ id: 'oa-relevant', from: 'OASIS', to: 'A', kind: 'observes' }],
    affordances: [{ id: 'observe-a', actor: 'OASIS', action: 'observe', target: 'A' }]
  }));
  const d = o.deliberate();
  assert.equal(d.field.currentRelations.some(r => r.id === 'xy-unrelated'), false);
  assert.equal(d.field.currentRelations.some(r => r.id === 'oa-relevant'), true);
}

const fs = await import('node:fs/promises');
const source = (await fs.readFile(new URL('../src/oasis-core.mjs', import.meta.url), 'utf8')) + '\n' + (await fs.readFile(new URL('../src/oasis-integrated-core.mjs', import.meta.url), 'utf8'));
