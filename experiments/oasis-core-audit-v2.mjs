...
for(const f of ['prehistoric-society-v3-core.js','prehistoric-society-v3-decisions.js','prehistoric-society-v3-world.js','oasis-core-v2.js']){
...
// Frozen semantic check 1: relation recombination must be able to generate and select a candidate not prelisted in primitive affordances.
{
  ...
  add('new_candidate_generation',true,!input.has(actionKey(d.a)),{chosen:actionKey(d.a),input:[...input],generated:!!d.a.generated,steps:d.a.steps?.map(actionKey)||[]});
}

// Frozen semantic check 2: structurally same relation must reactivate even when the concrete object ID changed.
{
  ...
  const active=activeRelations(A,P);
  add('structural_reactivation_different_id',true,active.length>0,{active:active.length,roles:active[0]?._roles||[]});
}

// Frozen semantic check 3: current relevance must not be discarded solely because of a fixed age window.
{
  const P=baseP();P.nearNodes=[{id:'newfire',kind:'natural_fire',x:10,y:10,stock:1,d:14}];
  A.relationEpisodes=[{t:3000,action:'study_fire',context:['node:newfire'],sig:'aged',outcome:.5}];
  const active=activeRelations(A,P);
  add('no_fixed_age_cutoff_for_currently_relevant_relation',true,active.length>0,{age:E.tick-3000,active:active.length});
}

// Frozen semantic check 4: simultaneous relation-set size must change the actual combinatorial term.
{
...
// Frozen semantic check 5: choice/responsibility must interact; responsibility may dominate under flow collapse but is not an immutable first tuple slot.
{
...
// Frozen semantic check 6: repeated relational processes are append-only and keep sequence links rather than replacing the older episode.
{
  const P=baseP();A.relationEpisodes=[];A.oasisV2={sequence:0,intentSeq:0,generatedUses:{},relationGraph:{},lastIntent:null};
  execute(A,P,{type:'observe'});E.tick++;execute(A,P,{type:'observe'});
  const ok=A.relationEpisodes.length===2&&A.relationEpisodes[0].sequence===1&&A.relationEpisodes[1].sequence===2&&A.relationEpisodes[1].previousSequence===1;
  add('preserve_repeated_relational_sequence',true,ok,{length:A.relationEpisodes.length,sequences:A.relationEpisodes.map(x=>x.sequence),previous:A.relationEpisodes.map(x=>x.previousSequence)});
}

globalThis.__AUDIT_V2_TESTS__=tests;
`);

const tests=globalThis.__AUDIT_V2_TESTS__;
