...
import { createCleanPrehistoricSocietyV1_1 } from './prehistoric-clean-world-v1.1-gamma-v2.1-audited.mjs';
...
import {
  OASISGammaV21Kernel,
  OASISGammaV21OffKernel,
  OASISGammaV21OrderScrambledKernel,
  OASISGammaV21CandidateMatchedShamKernel,
  bundle, assertCommon
} from './oasis-gamma-v2.1-kernel.mjs';

const SEED_SLOT = Number(process.env.OASIS_GAMMA_V21_SEED_SLOT || 0);
const ELIGIBILITY_MAX_CYCLES = 300;
const POST_CYCLES = 100;
...
if (!Number.isInteger(SEED_SLOT) || SEED_SLOT < 0 || SEED_SLOT > 39) throw new Error('OASIS_GAMMA_V21_SEED_SLOT must be 0..39');
...
const CODE_SHA = process.env.GITHUB_SHA || execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim();
...
function fixedSeedTable() {
  return Array.from({ length: 40 }, (_, sourceIndex) => {
    const seed = `oasis-gamma-causal-v2.1:${String(sourceIndex).padStart(3, '0')}`;
    return { sourceIndex, seed, hash: shaText(seed) };
  }).sort((a, b) => a.hash.localeCompare(b.hash) || a.sourceIndex - b.sourceIndex);
}

export const FIXED_GAMMA_V21_SEEDS = Object.freeze(fixedSeedTable());
const seedRow = FIXED_GAMMA_V21_SEEDS[SEED_SLOT];
...
function createActors(society, KernelClass = OASISGammaV21Kernel) {
...
function causalDigest(society, actors) {
  return sha({ world: society.auditSnapshot(), actors: actorStates(actors) });
}
...
function previewBundle(KernelClass, sourceActor, society, spec, observation, interventionActive = false) {
  ...
  const complete = bundle(scratch, observation, BUDGET);
  return {...complete, admissible:complete.life.admissible, diagnostics:clone(scratch.lastGammaV2Diagnostics)};
}
...
function previewEligibility(society, actors, spec) {
  ...
  const on = previewBundle(OASISGammaV21Kernel, actor, society, spec, observation, false);
  const off = previewBundle(OASISGammaV21OffKernel, actor, society, spec, observation, true);
...
async function locateOnset() {
  ...
  const actors = createActors(society, OASISGammaV21Kernel);
...
function branchDefinitions() {
  return [
    { id: 'G0', label: 'intact-v2', KernelClass: OASISGammaV21Kernel },
    { id: 'G1', label: 'gamma-off-v2', KernelClass: OASISGammaV21OffKernel },
    { id: 'G2', label: 'order-scrambled-history-control', KernelClass: OASISGammaV21OrderScrambledKernel },
    { id: 'G3', label: 'candidate-count-matched-sham', KernelClass: OASISGammaV21CandidateMatchedShamKernel }
  ];
}

function activateBranch(actors, branchId) {
  // G0 also activates read-only conformance checks.
...
async function replayBranch(definition, onset) {
  ...
  for (let cycleIndex = 0; cycleIndex < finalCycleIndex; cycleIndex++) {
    ...
    for (let orderIndex = 0; orderIndex < order.length; orderIndex++) {
      ...
      if (atOnset) {
        ...
        assert.deepStrictEqual({world:society.auditSnapshot(),actors:actorStates(actors)},onset.preWorldActorRng,'PRE_INTERVENTION_WORLD_ACTOR_RNG_DEEP_EQUALITY');
  ...
  return {
    conformanceAuditCount: [...actors.values()].reduce((n,a)=>n+a.auditCount,0),
...
const result = {
  protocol: 'OASIS Gamma Causal Probe v2.1',
  status: 'completed',
  codeSha: CODE_SHA,
  designLock: 'OASIS_GAMMA_V2.1_DESIGN_LOCK.md',
  preregistration: 'OASIS_GAMMA_V2.1_PREREGISTRATION.md',
  seedSlot: SEED_SLOT,
...
const filename = `oasis-gamma-v2.1-causal-probe-seed-slot-${SEED_SLOT}.json`;
