...
class AttackTests(unittest.TestCase):
    ...
    def test_inv_01_02_03_gap_gets_only_full_semantic_trace(self):
        ...
        d=Detector(); h,_,_=self.build(gap=d); f=MutableFlow(); p=h.execute_decision_epoch(f,episode_id="x"); self.close(h,p,f)
    ...
    def test_inv_04_real_flow_change_causes_no_to_yes(self):
        h,_,_=self.build(); f=MutableFlow(); no=h.execute_decision_epoch(f,episode_id="r"); self.assertEqual(no.branch,"NO"); self.close(h,no,f)
    ...
    def test_inv_07_08_nonparticipant_never_reaches_core_but_audit_keeps_it(self):
        core=CanaryCore(); h,ctx,_=self.build(core=core); f=MutableFlow(); a=h.execute_decision_epoch(f,episode_id="r"); self.close(h,a,f); f.next_epoch(1)
    ...
    def test_inv_15_requires_exact_complete_revalidation(self):
        ...
        h,_,_=self.build(revalidate=RV()); f=MutableFlow(); p=h.execute_decision_epoch(f,episode_id="r"); self.close(h,p,f); f.next_epoch(1); p=h.execute_decision_epoch(f,episode_id="r")
        with self.assertRaises(GovernanceInvariantError): self.close(h,p,f)
    ...
    def test_inv_19_yes_all_no_is_valid_current_only(self):
        ...
        core=CanaryCore(); h,_,_=self.build(core=core,reengage=AllNo()); f=MutableFlow(); p=h.execute_decision_epoch(f,episode_id="r"); self.close(h,p,f); f.next_epoch(1); p=h.execute_decision_epoch(f,episode_id="r"); self.assertEqual(core.history_reads[-1],frozenset())

    def test_inv_20_complete_metrics(self):
        h,_,_=self.build(); f=MutableFlow(); p=h.execute_decision_epoch(f,episode_id="r"); self.close(h,p,f); f.next_epoch(1); p=h.execute_decision_epoch(f,episode_id="r")
