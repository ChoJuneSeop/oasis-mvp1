...
echo OF-01 A1 is preserved as a PRE_FIRST_TICK_FAIL lineage; this launcher uses A2.
echo It enters PRE_FIRST_TICK_HOLD and waits for explicit release before empirical tick 1.
echo.
python -m host_tools.g3_rpfo_carla_01.external_carla_runner --flow OF-01 --attempt 2 --output-root "%OUTPUT_ROOT%" --host 127.0.0.1 --port 2000
set "RC=%ERRORLEVEL%"
echo.
echo Exit code: %RC%
pause
